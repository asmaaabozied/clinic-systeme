...
class DentalChart {
    ...
    openToothModal(toothNumber) {
        ...
        const toothNumberInput = document.getElementById('tooth-number-input');
        // Prefill modal fields
        const toothElement = document.querySelector(`.tooth-element[data-tooth-number='${toothNumber}']`);
        let toothData = {};
        if (toothElement) {
            const dataTooth = toothElement.dataset.tooth;
            if (dataTooth && dataTooth !== 'null' && dataTooth !== '{}') {
                try {
                    toothData = JSON.parse(dataTooth);
        ...
        // Set modal fields with null checks
        const conditionInput = document.getElementById('tooth-condition');
        if (conditionInput) {
            conditionInput.value = toothData.condition || 'healthy';
        }
        const severityInput = document.getElementById('tooth-severity');
        if (severityInput) {
            severityInput.value = toothData.severity || 'mild';
        }
        const dateInput = document.getElementById('tooth-date');
        if (dateInput) {
            dateInput.value = toothData.date || '';
        }
        const notesInput = document.getElementById('tooth-notes');
        if (notesInput) {
            notesInput.value = toothData.notes || '';
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // After form submit, reload page to update chart color
    const form = document.querySelector('#tooth-modal form');
    if (form) {
...
// Legacy helpers for dynamic fields used in modal forms
window.addIssueField = function() {
    ...
    const html = `
        <div class="flex items-center gap-2 mb-2 fade-in">
            <input type="text" name="issues[]" class="flex-1 border border-gray-300 rounded-md px-3 py-2" placeholder="Enter issue...">
            <button type="button" onclick="removeField(this)" class="text-red-600 hover:text-red-700 p-2 hover:bg-red-50 rounded-lg">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
            </button>
...
window.addRiskFactorField = function() {
    const container = document.getElementById('risk-factors-container');
    if (!container) return;
    const html = `
        <div class="flex items-center gap-2 mb-2 fade-in">
            <input type="text" name="risk_factors[]" class="flex-1 border border-gray-300 rounded-md px-3 py-2" placeholder="Enter risk factor...">
            <button type="button" onclick="removeField(this)" class="text-red-600 hover:text-red-700 p-2 hover:bg-red-50 rounded-lg">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
...
window.addRecommendationField = function() {
