...
class UserController extends Controller
{
    ...
    public function create()
    {
        ...
        $specializations = DoctorSpecialization::pluck('name', 'id');
        if (\Auth::user()->can('create user')) {
            return view('user.create', compact('roles', 'customFields', 'specializations'));
        } else {
            return redirect()->back();
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create user')) {
            ...
            if (\Auth::user()->type == 'super admin') {
                ...
                $roleObj = Role::find($request->role);
                if ($roleObj && $roleObj->name == 'Doctor') {
                    $rules['doctor_specialization_id'] = 'required|exists:doctor_specializations,id';
                }
            ...
            //if Doctor create Doctor mode
            if ($user->type == 'Doctor') {
                $doctor = new Doctor();

                $doctor->user_id = $user->id;
                $doctor->doctor_code = rand(1000, 9999);
                $doctor->specialization_id = $request->doctor_specialization_id;
                $doctor->save();
            }
            // Send Email
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit user')) {
            ...
            return view('user.edit', compact('user', 'roles', 'customFields', 'specializations'));
        } else {
            return redirect()->back();
        }
    ...
    public function editprofile(Request $request)
    {
        ...
        if ($image = $request->file('profile')) {
            ...
            $user['avatar'] = store_file($image, 'avatar');
        }
