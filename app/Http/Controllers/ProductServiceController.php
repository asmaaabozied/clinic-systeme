...
class ProductServiceController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (\Auth::user()->can('manage product & service')) {
            ...
            $brands->prepend('Select Brand', '');
            $branchs = Branch::get()->pluck('name', 'id');
            ...
            if (!empty($request->category)) {
                ...
                $productServices = ProductService::where('brand_id', $request->brand_id)->with(['category', 'unit', 'brand'])->get();

            } elseif (!empty($request->branch_id)) {

                $productServices = ProductService::where('branch_id', $request->branch_id)->with(['category', 'unit', 'brand'])->get();

            } elseif (!empty($request->unit_id)) {

                $productServices = ProductService::where('unit_id', $request->unit_id)->with(['category', 'unit', 'brand'])->get();

            } elseif (!empty($request->tax_id)) {

                $productServices = ProductService::where('tax_id', $request->tax_id)->with(['category', 'unit', 'brand'])->get();

            } elseif (!empty($request->type_product)) {

                $productServices = ProductService::where('type_product', $request->type_product)->with(['category', 'unit', 'brand'])->get();

            } else {
                $productServices = ProductService::where('created_by', '=', \Auth::user()->creatorId())->with(['category', 'unit'])->get();
            }

            $productStock = ProductService::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'product')->get();

            return view('productservice.index', compact('productServices', 'productStock','units', 'brands', 'tax', 'branchs', 'category'));
        } else {
    ...
    public function create()
    {
        if (\Auth::user()->can('create product & service')) {
            ...
            return view('productservice.create', compact('category', 'branch', 'brand', 'unit', 'tax', 'customFields', 'incomeChartAccounts', 'incomeSubAccounts', 'expenseChartAccounts', 'expenseSubAccounts'));
        } else {
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create product & service')) {
            ...
            $productService->expire_date = $request->expire_date;
            ...
            $productService->branch_id = $request->branch_id;
            $productService->brand_id = $request->brand_id;
            $productService->quantity2 = $request->quantity2;
            $productService->code = $request->code;
            $productService->weight = $request->weight;
            $productService->field1 = $request->field1;
            $productService->field2 = $request->field2;
            $productService->field3 = $request->field3;
            $productService->field4 = $request->field4;
            $productService->type_sales = $request->type_sales;
            $productService->vat = $request->vat;
            $productService->type_product = $request->type_product;
//            $productService->sell_price = $request->sell_price;
//            $productService->percentage = $request->percentage;
//            $productService->tax_not = $request->tax_not;
//            $productService->tax = $request->tax;


            if (!empty($request->pro_image)) {

                $thumbnail = $request->file('pro_image');
            ...
            if (!empty($request->image2)) {
                //storage limit
                $thumbnail = $request->file('image2');
                $destinationPath = 'uploads/products/';
                $filename = time() . '.' . $thumbnail->getClientOriginalExtension();
                $thumbnail->move($destinationPath, $filename);
                $productService->image2 = $filename;
                $productService->save();
            }
    ...
    {
        $products=ProductService::find($id);
