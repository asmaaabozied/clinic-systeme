...
class SystemController extends Controller
{
    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if (\Auth::user()->can('manage business settings')) {
            ...
            $user = \Auth::user();
            if ($image = $request->company_logo_dark) {
           
                $logoName = store_file($image, 'company_logo_dark');
                \DB::insert(
                    'insert into settings (`value`, `name`,`created_by`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ',
                    [
                        $logoName,
            ...
            if ($image = $request->company_logo_light) {
           
                $logoName = store_file($image, 'company_logo_light');
                \DB::insert(
                    'insert into settings (`value`, `name`,`created_by`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ',
                    [
                        $logoName,
            ...
            if ($image = $request->company_favicon) {
           
                $logoName = store_file($image, 'company_favicon');
                \DB::insert(
                    'insert into settings (`value`, `name`,`created_by`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ',
                    [
                        $logoName,
