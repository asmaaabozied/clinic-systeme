{{ Form::open(array('url' => 'productservice','enctype' => "multipart/form-data", 'class'=>'needs-validation', 'novalidate')) }}
<div class="modal-body">
    {{-- start for ai module--}}
    @php
        $plan= \App\Models\Utility::getChatGPTSettings();
    @endphp
    @if($plan->chatgpt == 1)
        <div class="text-end mb-3">
            <a href="#" data-size="md" class="btn  btn-primary btn-icon btn-sm" data-ajax-popup-over="true" data-url="{{ route('generate',['productservice']) }}"
               data-bs-placement="top" data-title="{{ __('Generate content with AI') }}">
                <i class="fas fa-robot"></i> <span>{{__('Generate with AI')}}</span>
            </a>
        </div>
    @endif
    {{-- end for ai module--}}
    <div class="row">
        <div class="col-md-4">
            <div class="form-group">
                {{ Form::label('name', __('Name'),['class'=>'form-label']) }}<x-required></x-required>
                {{ Form::text('name', '', array('class' => 'form-control','required'=>'required', 'placeholder' => __('Enter Name'))) }}
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                {{ Form::label('sku', __('SKU'),['class'=>'form-label']) }}<x-required></x-required>
                {{ Form::text('sku', '', array('class' => 'form-control','required'=>'required', 'placeholder' => __('Enter Sku'))) }}
            </div>

        </div>
        <div class="col-md-4">
            <div class="form-group">
                {{ Form::label('type BarCode', __('Type BarCode'),['class'=>'form-label']) }}<x-required></x-required>
                {{ Form::text('code', auth()->user()->barcodeType(), array('class' => 'form-control','required'=>'required','value'=>auth()->user()->barcodeType(), 'placeholder' => __('Enter Code'))) }}
            </div>
        </div>
        <div class="form-group col-md-4">
            {{ Form::label('unit_id', __('Unit'),['class'=>'form-label']) }}<x-required></x-required>
            {{ Form::select('unit_id', $unit,null, array('class' => 'form-control select2','required'=>'required')) }}
            <div class=" text-xs mt-1">
                {{__('Create unit here. ')}}<a href="{{route('product-unit.index')}}"><b>{{__('Create unit')}}</b></a>
            </div>
        </div>
        <div class="form-group col-md-4">
            {{ Form::label('brand_id', __('Brands'),['class'=>'form-label']) }}<x-required></x-required>
            {{ Form::select('brand_id', $brand ,null, array('class' => 'form-control select2','required'=>'required')) }}
            <div class=" text-xs mt-1">
                {{__('Create Brand here. ')}}<a href="{{route('brands.index')}}"><b>{{__('Create Brand')}}</b></a>
            </div>
        </div>        <div class="form-group col-md-4">
            {{ Form::label('branch_id', __('Branchs'),['class'=>'form-label']) }}<x-required></x-required>
            {{ Form::select('branch_id', $branch ,null, array('class' => 'form-control select2','required'=>'required')) }}

        </div>

        <div class="form-group col-md-4">
            {{ Form::label('category_id', __('Category'),['class'=>'form-label']) }}<x-required></x-required>
            {{ Form::select('category_id', $category,null, array('class' => 'form-control select2','required'=>'required')) }}

            <div class=" text-xs mt-1">
                {{__('Create category here. ')}}<a href="{{route('product-category.index')}}"><b>{{__('Create Category')}}</b></a>
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                {{ Form::label('quantity2', __('Quantity alert'),['class'=>'form-label']) }}<x-required></x-required>
                {{ Form::number('quantity2', '', array('class' => 'form-control','step'=>'0.01', 'placeholder' => __('Enter Sale Price'))) }}
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                {{ Form::label('sale_price', __('Sale Price'),['class'=>'form-label']) }}<x-required></x-required>
                {{ Form::number('sale_price', '', array('class' => 'form-control','required'=>'required','step'=>'0.01', 'placeholder' => __('Enter Sale Price'))) }}
            </div>
        </div>
        <div class="form-group col-md-4">
            {{ Form::label('sale_chartaccount_id', __('Income Account'),['class'=>'form-label']) }}<x-required></x-required>
            <select name="sale_chartaccount_id" class="form-control select2" required="required">
                @foreach ($incomeChartAccounts as $key => $chartAccount)
                    <option value="{{ $key }}" class="subAccount">{{ $chartAccount }}</option>
                    @foreach ($incomeSubAccounts as $subAccount)
                        @if ($key == $subAccount['account'])
                            <option value="{{ $subAccount['id'] }}" class="ms-5"> &nbsp; &nbsp;&nbsp; {{ $subAccount['code_name'] }}</option>
                        @endif
                    @endforeach
                @endforeach
            </select>
            <div class="text-xs mt-1">
                {{ __('Create account here.') }} <a href="{{ route('chart-of-account.index') }}"><b>{{ __('Create account') }}</b></a>
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                {{ Form::label('purchase_price', __('Purchase Price'),['class'=>'form-label']) }}<x-required></x-required>
                {{ Form::number('purchase_price', '', array('class' => 'form-control','required'=>'required','step'=>'0.01', 'placeholder' => __('Enter Purchase Price'))) }}
            </div>
        </div>
        <div class="form-group col-md-4">
            {{ Form::label('expense_chartaccount_id', __('Expense Account'),['class'=>'form-label']) }}
            <select name="expense_chartaccount_id" class="form-control select2" required="required">
                @foreach ($expenseChartAccounts as $key => $chartAccount)
                    <option value="{{ $key }}" class="subAccount">{{ $chartAccount }}</option>
                    @foreach ($expenseSubAccounts as $subAccount)
                        @if ($key == $subAccount['account'])
                            <option value="{{ $subAccount['id'] }}" class="ms-5"> &nbsp; &nbsp;&nbsp; {{ $subAccount['code_name'] }}</option>
                        @endif
                    @endforeach
                @endforeach
            </select>
            <div class="text-xs mt-1">
                {{ __('Create account here.') }} <a href="{{ route('chart-of-account.index') }}"><b>{{ __('Create account') }}</b></a>
            </div>
        </div>

        <div class="col-md-4 form-group">
            {{ Form::label('tax_id', __('Tax'),['class'=>'form-label']) }}
            {{ Form::select('tax_id[]', $tax,null, array('class' => 'form-control select2','id'=>'choices-multiple1')) }}
            <div class=" text-xs mt-1">
                {{__('Create tax here. ')}}<a href="{{route('taxes.index')}}"><b>{{__('Create tax')}}</b></a>
            </div>
        </div>


        <div class="form-group col-md-4 quantity">
            {{ Form::label('quantity', __('Quantity'),['class'=>'form-label']) }}<x-required></x-required>
            {{ Form::text('quantity',null, array('class' => 'form-control', 'required'=>'required', 'placeholder' => __('Enter Quantity'))) }}
        </div>


        <div class="col-md-4 form-group">
            {{Form::label('pro_image',__('Product Image'),['class'=>'form-label'])}}
            <div class="choose-file ">
                <label for="pro_image" class="form-label">
                    <input type="file" class="form-control file-validate" name="pro_image" id="pro_image" data-filename="pro_image_create">
                    <p id="" class="file-error text-danger"></p>
                    <img id="image" class="mt-3" style="width:25%;"/>
                </label>
            </div>
        </div>
        <div class="col-md-4 form-group">
            {{Form::label('img',__('Product brochure'),['class'=>'form-label'])}}
            <div class="choose-file ">
                <label for="img" class="form-label">
                    <input type="file" class="form-control file-validate" name="img" id="img" data-filename="pro_image_create">
                    <p id="" class="file-error text-danger"></p>
                    <img id="imgs" class="mt-3" style="width:25%;"/>
                </label>
            </div>
        </div>
        <div class="form-group col-md-4">
            <div class="form-group">
                <div class="btn-box">
                    <label class="d-block form-label">{{__('Type')}}</label>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-check form-check-inline">
                                <input type="radio" class="form-check-input type" id="customRadio5" name="type" value="product" checked="checked" >
                                <label class="custom-control-label form-label" for="customRadio5">{{__('Product')}}</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-check form-check-inline">
                                <input type="radio" class="form-check-input type" id="customRadio6" name="type" value="service" >
                                <label class="custom-control-label form-label" for="customRadio6">{{__('Service')}}</label>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="form-group col-md-6">
            {{ Form::label('description', __('Description'),['class'=>'form-label']) }}
            {!! Form::textarea('description', null, ['class'=>'form-control','rows'=>'2', 'placeholder' => __('Enter Description')]) !!}
        </div>


        <hr>


        <div class="form-group col-md-4 ">
            {{ Form::label('weight', __('Weight'),['class'=>'form-label']) }}
            {{ Form::text('weight',null, array('class' => 'form-control', 'placeholder' => __('Enter Weight'))) }}
        </div>


        <div class="form-group col-md-4 ">
            {{ Form::label('field1', __('Custom Field 1'),['class'=>'form-label']) }}
            {{ Form::text('field1',null, array('class' => 'form-control', 'placeholder' => __('Enter Custom Field 1'))) }}
        </div>

        <div class="form-group col-md-4 ">
            {{ Form::label('field2', __('Custom Field 2'),['class'=>'form-label']) }}
            {{ Form::text('field2',null, array('class' => 'form-control', 'placeholder' => __('Enter Custom Field 2'))) }}
        </div>
        <div class="form-group col-md-4 ">
            {{ Form::label('field3', __('Custom Field 3'),['class'=>'form-label']) }}
            {{ Form::text('field3',null, array('class' => 'form-control', 'placeholder' => __('Enter Custom Field 3'))) }}
        </div>
        <div class="form-group col-md-4 ">
            {{ Form::label('field4', __('Custom Field 4'),['class'=>'form-label']) }}
            {{ Form::text('field4',null, array('class' => 'form-control', 'placeholder' => __('Enter Custom Field 4'))) }}
        </div>

        <hr>


        <div class="form-group col-md-4">
            {{ Form::label('vat', __('Applicable tax'),['class'=>'form-label']) }}
            {{ Form::select('vat', ['NoOne'=>'NoOne','Vat'=>'Vat'],null, array('class' => 'form-control select2')) }}

        </div>
        <div class="form-group col-md-4">
            {{ Form::label('type_sales', __('Sales tax type'),['class'=>'form-label']) }}
            {{ Form::select('type_sales', ['Except'=>'Except','Including'=>'Including'],null, array('class' => 'form-control select2')) }}

        </div>

        <div class="form-group col-md-4">
            {{ Form::label('type_product', __('Type Product'),['class'=>'form-label']) }}
            {{ Form::select('type_product', ['Individual'=>'Individual','Combo'=>'Combo','Contrasting'=>'Contrasting'],null, array('class' => 'form-control select2')) }}

        </div>

        <hr>
        {{--        <div class="form-group col-md-4 ">--}}
        {{--            {{ Form::label('tax_not', __('Tax Not Included'),['class'=>'form-label']) }}--}}
        {{--            {{ Form::text('tax_not',null, array('class' => 'form-control', 'placeholder' => __('Enter Tax Not Included'))) }}--}}
        {{--        </div>--}}

        {{--        <div class="form-group col-md-4 ">--}}
        {{--            {{ Form::label('tax', __('Tax Included'),['class'=>'form-label']) }}--}}
        {{--            {{ Form::text('tax',null, array('class' => 'form-control', 'placeholder' => __('Enter Tax Included'))) }}--}}
        {{--        </div>--}}

        {{--        <div class="form-group col-md-4 ">--}}
        {{--            {{ Form::label('percentage', __('Margin'),['class'=>'form-label']) }}--}}
        {{--            {{ Form::text('percentage',null, array('class' => 'form-control', 'placeholder' => __('Enter Margin'))) }}--}}
        {{--        </div>--}}
        {{--        <div class="form-group col-md-4 ">--}}
        {{--            {{ Form::label('sell_price', __('Tax Exclusion'),['class'=>'form-label']) }}--}}
        {{--            {{ Form::text('sell_price',null, array('class' => 'form-control', 'placeholder' => __('Enter Tax Exclusion'))) }}--}}
        {{--        </div>--}}
        {{--        <div class="col-md-4 form-group">--}}
        {{--            {{Form::label('image2',__('Product Image'),['class'=>'form-label'])}}--}}
        {{--            <div class="choose-file ">--}}
        {{--                <label for="image2" class="form-label">--}}
        {{--                    <input type="file" class="form-control file-validate" name="image2" id="image2" data-filename="pro_image_create">--}}
        {{--                    <p id="" class="file-error text-danger"></p>--}}
        {{--                    <img id="image22" class="mt-3" style="width:25%;"/>--}}
        {{--                </label>--}}
        {{--            </div>--}}
        {{--        </div>--}}


        @if(!$customFields->isEmpty())
            @include('customFields.formBuilder')
        @endif
    </div>
</div>
<div class="modal-footer">
    <input type="button" value="{{__('Cancel')}}" class="btn  btn-secondary" data-bs-dismiss="modal">
    <input type="submit" value="{{__('Create')}}" class="btn  btn-primary">
</div>
{{Form::close()}}


<script>
    document.getElementById('pro_image').onchange = function () {
        var src = URL.createObjectURL(this.files[0])
        document.getElementById('image').src = src
    }
    document.getElementById('img').onchange = function () {
        var src = URL.createObjectURL(this.files[0])
        document.getElementById('imgs').src = src
    }

    //hide & show quantity

    $(document).on('click', '.type', function ()
    {
        var type = $(this).val();
        if (type == 'product') {
            $('.quantity').removeClass('d-none')
            $('.quantity').addClass('d-block');
            $('input[name="quantity"]').prop('required', true);
        } else {
            $('.quantity').addClass('d-none')
            $('.quantity').removeClass('d-block');
            $('input[name="quantity"]').val('').prop('required', false);
        }
    });
</script>
