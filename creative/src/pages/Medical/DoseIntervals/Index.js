...
const DoseIntervalsIndex = () => {
  document.title = 'Dose Intervals | Medical Management';
  ...
  const fetchData = async (page = 1, perPage = 10, searchValue = '') => {
    setLoading(true);
    try {
      const response = await API.get(`/medical/dose-intervals?page=${page}&per_page=${perPage}`);
  ...
  const renderActions = (doseInterval) => (
    <div className="d-flex gap-2">
      <Button size="sm" color="primary" onClick={() => handleEdit(doseInterval.id, doseInterval.name)}>
        Edit
      </Button>
      <Button size="sm" color="danger" onClick={() => handleDelete(doseInterval.id)}>
        Delete
      </Button>
    </div>
  );
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    await API.delete(`/medical/dose-intervals/${deleteId}`);
  ...
  const handleUpdate = async (e) => {
    ...
    setEditError('');
    try {
      await API.put(`/medical/dose-intervals/${editId}`, { name: editName });
      setEditModalOpen(false);
      setEditId(null);
      setEditName('');
      fetchData(meta.current_page, meta.per_page, search);
    } catch (err) {
      setEditError(err.response?.data?.message || 'Failed to update dose interval');
    } finally {
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm('Are you sure you want to delete the selected dose intervals?')) {
      await Promise.all(ids.map(id => API.delete(`/medical/dose-intervals/${id}`)));
      setRowSelection({});
      fetchData(meta.current_page, meta.per_page, search);
    }
  };

  const handleAddDoseInterval = async (e) => {
    e.preventDefault();
    setAddLoading(true);
    setAddError('');
    try {
      await API.post('/medical/dose-intervals', { name: newName });
      setAddModalOpen(false);
      setNewName('');
      fetchData(meta.current_page, meta.per_page, search);
    } catch (err) {
      setAddError(err.response?.data?.message || 'Failed to add dose interval');
    } finally {
      setAddLoading(false);
    }
  };
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Dose Intervals" pageTitle="Medical Management" />
          <Row className="mb-3">
            <Col xs="auto">
          ...
          <Row>
            <Col xs={12}>
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Dose Intervals</h5>
                </CardHeader>
                <CardBody>
                  <TableContainer
                    columns={columns}
                    ...
                    isSelectable={true}
                    isSearched={true}
                    searchPlaceholder="Search for dose intervals..."
      ...
      <Modal isOpen={addModalOpen} toggle={() => setAddModalOpen(!addModalOpen)}>
        ...
        <Form onSubmit={handleAddDoseInterval}>
          <ModalBody>
            <FormGroup>
...
export default DoseIntervalsIndex; 