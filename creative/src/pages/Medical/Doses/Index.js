...
const DosesIndex = () => {
  document.title = 'Doses | Medical Management';
  ...
  const fetchData = async (page = 1, perPage = 10, searchValue = '') => {
    setLoading(true);
    try {
      const response = await API.get(`/medical/doses?page=${page}&per_page=${perPage}`);
  ...
  const renderActions = (dose) => (
    <div className="d-flex gap-2">
      <Button size="sm" color="primary" onClick={() => handleEdit(dose.id, dose.name)}>
        Edit
      </Button>
      <Button size="sm" color="danger" onClick={() => handleDelete(dose.id)}>
        Delete
      </Button>
    </div>
  );
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    await API.delete(`/medical/doses/${deleteId}`);
  ...
  const handleUpdate = async (e) => {
    ...
    setEditError('');
    try {
      await API.put(`/medical/doses/${editId}`, { name: editName });
      setEditModalOpen(false);
      setEditId(null);
      setEditName('');
      fetchData(meta.current_page, meta.per_page, search);
    } catch (err) {
      setEditError(err.response?.data?.message || 'Failed to update dose');
    } finally {
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm('Are you sure you want to delete the selected doses?')) {
      await Promise.all(ids.map(id => API.delete(`/medical/doses/${id}`)));
      setRowSelection({});
      fetchData(meta.current_page, meta.per_page, search);
    }
  };
  ...
  const handleAddDose = async (e) => {
    ...
    setAddError('');
    try {
      await API.post('/medical/doses', { name: newName });
      setAddModalOpen(false);
      setNewName('');
      fetchData(meta.current_page, meta.per_page, search);
    } catch (err) {
      setAddError(err.response?.data?.message || 'Failed to add dose');
    } finally {
      setAddLoading(false);
    }
  };
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Doses" pageTitle="Medical Management" />
          <Row className="mb-3">
            <Col xs="auto">
          ...
          <Row>
            <Col xs={12}>
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Doses</h5>
                </CardHeader>
                <CardBody>
                  <TableContainer
                    columns={columns}
                    ...
                    isSelectable={true}
                    isSearched={true}
                    searchPlaceholder="Search for doses..."
